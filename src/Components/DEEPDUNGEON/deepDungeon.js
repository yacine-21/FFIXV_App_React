...
const deepDungeon = () => {
    ...
    const onClickHandlerDeepDungeon = async (e) =>{
        e.preventDefault();
    ...
    if(resultOne!== undefined && click === true){
    ...
    return(
        <>
            ...
            <div className={styles.container}>
                <h1>DeepDungeon</h1>
                {click === false ? <button className={styles.buttonCardShow} onClick={onClickHandlerDeepDungeon}>SHOW DEBUFF</button> : null}
                <ul>
                    <li>The Palace of the Dead 1 - 60</li>
                    <li>DEBUFF  : DeepDungeonBan</li>
                    <li> brume du néant :DeepDungeonDanger/1</li>
                    {click === true ? resultOne === undefined ? null : 
                                ...
                                <ul key={danger.ID}>
                                        <>
                                            <Card className={styles.card}>
                                                    <Card.Img className={styles.miniature} src={`https://xivapi.com${danger.Name.IconHD}`}/>
                                                <Card.Body className={styles.bodyCard}>
                                                    <Card.Title>{<li>{danger.Name.Name_en}</li>}</Card.Title>
                                                    <Card.Text>
                        ...
                        {resultTwo === undefined ? null : 
                            <Card className={styles.card}>
                                    <Card.Img className={styles.miniature} src={`https://xivapi.com${resultTwo.Results[0].Name.IconHD}`}/>
                                <Card.Body className={styles.bodyCard}>
                                    <Card.Title>{<li>{resultTwo.Results[0].Name.Name_en}</li>}</Card.Title>
                                    <Card.Text>
                ...
                {click === true ? <button className={styles.buttonCardShow} onClick={onClickHandlerDeepDungeonHide}>HIDE DEBUFF</button> : null}
            </div>
        </>
    )
}
